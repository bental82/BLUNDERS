...
var S={order:[],k:0,ply:0,phase:"idle",failed:false,hinted:false,parts:[],
       pendAnn:null,explore:null,trapAlt:null,trapK:0,trapAuto:false,timer:null,
       why:"",ready4:-1,marked:false};
...
/* The dot is the worst of a position's quiz moves, so when it stays red this
   is what is holding it: the moves still owed a clean answer, and whether
   each is far enough past its last miss to be earned back today. */
function outstanding(it){
  var out=[],ord=0,i,r,pd;
  if(it.kind!=="pos")return out;
  for(i=0;i<it.plies.length;i++){
    if(!it.plies[i].q)continue;
    ord++;
    r=resOf(it.id+":"+i);
    if(r!=="missed"&&r!=="hint")continue;
    pd=pending(it.id+":"+i);
    out.push({ply:i,ord:ord,ready:!!(pd&&pd.ready),
              at:pd?readyAt(pd.since):0});
  }
  return out;
}
/* The first instant a clean answer would satisfy both halves of redeems():
   six hours past the miss, and past midnight of the day it happened. */
function readyAt(tBad){
  var mid=new Date(tBad);mid.setHours(24,0,0,0);
  return Math.max(tBad+REDEEM_MS,mid.getTime());
}
function readyPhrase(ms){
  var now=Date.now(),d=new Date(ms);
  if(ms<=now)return "now";
  var hm=("0"+d.getHours()).slice(-2)+":"+("0"+d.getMinutes()).slice(-2);
  var at=(hm==="00:00")?"":" at "+hm;      /* midnight needs no clock time */
  var tm=new Date(now);tm.setHours(24,0,0,0);
  if(dayOf(ms)===dayOf(now))return "later today"+at;
  if(dayOf(ms)===dayOf(tm.getTime()))return "tomorrow"+at;
  return ["Sunday","Monday","Tuesday","Wednesday","Thursday","Friday",
          "Saturday"][d.getDay()]+at;
}
function quizCount(it){
  var n=0;for(var i=0;i<it.plies.length;i++)if(it.plies[i].q)n++;return n;
}
/* "move 3" / "moves 3 and 4" / "moves 1, 3 and 4" */
function movesPhrase(list){
  var n=list.map(function(o){return o.ord});
  if(!n.length)return"";
  if(n.length===1)return "move "+n[0];
  return "moves "+n.slice(0,-1).join(", ")+" and "+n[n.length-1];
}
...
function renderRail(){
  ...
  DATA.forEach(function(it,i){
    ...
    var st2=statusOf(it),lf2=owedOn(it,st2);
    h+='<div class="row'+(it.kind!=="pos"?" lesson":"")+(st2?" s-"+st2:"")+
       '" data-i="'+i+'"><span class="dot"></span><span class="nm">'+
       esc(it.title)+"</span>"+
       (lf2?'<span class="owed" title="moves still to clear">'+lf2+"</span>":"")+"</div>";
  });
  list.innerHTML=h;
...
/* How many of a position's moves are still owed a clean answer. Only worth
   showing when a position has several -- on a one-move item the dot says it. */
function owedOn(it,st){
  if(st===undefined)st=statusOf(it);
  if((st!=="missed"&&st!=="hint")||quizCount(it)<2)return 0;
  return outstanding(it).length;
}
function syncRail(){
  ...
  for(var j=0;j<rows.length;j++){
    ...
    rows[j].className="row"+(it.kind!=="pos"?" lesson":"")+(st?" s-"+st:"")+
      (i===S.order[S.k]?" cur":"");
    var b=rows[j].querySelector(".owed"),n=owedOn(it,st);
    if(n){
      if(!b){b=document.createElement("span");b.className="owed";
             b.title="moves still to clear";rows[j].appendChild(b)}
      if(b.textContent!==String(n))b.textContent=n;
    } else if(b)rows[j].removeChild(b);
  }
...
function playTrap(p,alt){
  S.phase="trap";S.trapAlt=alt;S.trapK=0;S.trapAuto=true;
  board.set(p.f);
  var i=0;
  (function run(){
    if(S.phase!=="trap"||!S.trapAuto)return;
    if(i>=alt.line.length){renderPanel();return}
    board.move(alt.line[i].u);
    var st=alt.line[i];
    board.annotate(st.cal||[],st.csl||[]);
    S.trapK=i;i++;renderPanel();
    /* This is the explanation, not a transition -- it needs reading time. */
    setTimeout(run,d(st.t?2100:1200));
  })();
}
...
function renderPanel(){
  ...
  if(S.phase==="intro"||S.phase==="quiz"||S.phase==="idle"||S.phase==="ready"){
    ...
    h+=redeemNote(true);      /* right, but not always enough to clear the red */
    ...
    var qs=quizCount(it),left=outstanding(it),y;
    if(!left.length){
      h+='<div class="verdict v-ok"><span class="mk">'+ICO.ok+"</span>Position complete</div>";
      if(qs>1)h+='<div class="prose">All '+qs+" moves found.</div>";
    } else {
      /* Saying "position complete" over a red dot is what makes the dot look
         broken. Name what is still owed instead. */
      var many=left.length>1,soon=left[0].at;
      for(y=1;y<left.length;y++)if(left[y].at<soon)soon=left[y].at;
      h+='<div class="verdict v-nu"><span class="mk">'+ICO.nu+"</span>Still marked missed</div>";
      h+='<div class="prose">'+(qs>1
        ? cap(movesPhrase(left))+(many?" are":" is")+
          " still red — the dot goes green only once every move here is clean."
        : "This move is still red.")+"</div>";
      h+='<div class="hintline">'+(many?"They come":"It comes")+" back into play "+
         readyPhrase(soon)+" — answer "+(many?"them":"it")+" right then and the "+
         "red clears.</div>";
    }
...
function redeemNote(justRight){
  var pd=pending(item().id+":"+S.ply);
  if(!pd)return"";
  if(pd.ready&&!justRight)
    return '<div class="hintline">Answer this clean now and it goes back to green.</div>';
  return '<div class="hintline">'+
    (justRight?"Right \u2014 but you were shown this one moments ago, so it stays red. ":"")+
    "Answer it clean "+readyPhrase(readyAt(pd.since))+" and it goes back to green "+
    "\u2014 getting it right again straight after being shown proves recall of "+
    "the last minute, not of the pattern.</div>";
}
...
function wirePanel(){
  ...
  pn.querySelectorAll(".lm").forEach(function(b){
    b.addEventListener("click",function(e){
      ...
      if(ns==="trap"){
        /* Stepping the refutation by hand. Taking control stops the replay,
           otherwise the next tick would move the board out from under you. */
        var at=S.trapAlt;if(!at||st<0)return;
        S.trapAuto=false;S.trapK=st;
        var pt=item().plies[S.ply];
        board.set(pt.f);
        for(var j=0;j<=st;j++)board.move(at.line[j].u);
        board.annotate(at.line[st].cal||[],at.line[st].csl||[]);
        renderPanel();return;
      }
...
function buttons(){
  ...
  /* The secondary action, whatever it is in this phase. After a trap that is
     another go at the position -- the refutation showed why your move loses,
     which is not the same as being told the right one. */
  $("bReveal").style.display=(q||ms||tr)?"":"none";
  $("bHint").disabled=S.hinted;
  $("bReveal").textContent=tr?"Try again":((ms||S.hinted)?"Show me":"Give up");
  var n=$("bNext");
...
window.__CT={S:S,board:board,sync:cloudSync,absorb:absorb,begin:begin,
  resOf:resOf,pending:pending,statusOf:statusOf,outstanding:outstanding,owedOn:owedOn,
